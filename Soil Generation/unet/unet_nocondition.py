...
#     def __init__(self, vector_size, hidden_dim=16, num_heads=4, text_embed_dim=1024):
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, text_embed):
        # x: (B, seq_len, vector_size)
        x_norm = self.normalization(x)
        q = self.to_q(x_norm)

        if text_embed is None or text_embed.shape[0] != x.shape[0]:
            raise ValueError(f"Text embedding batch size {text_embed.shape[0]} does not match input {x.shape[0]}")

        # value_combiner: (B, 1, vector_size)
        combined_embeds_tensor = self.value_combiner(text_embed)

        # 使用 expand 而非 repeat，自动广播到 seq_len
        combined_embeds_tensor = combined_embeds_tensor.expand(-1, x.shape[1], -1)
        combined_embeds_norm = self.condition_normalization(combined_embeds_tensor)

        # k/v 与 q 具有相同 seq_len
        k = self.to_k(combined_embeds_norm)
        v = self.to_v(combined_embeds_norm)
...
class Block(nn.Module):
    def __init__(self, n_inputs, n_outputs, number_of_diffusions,
                 kernel_size=5, n_heads=4, hidden_dim=16, text_embed_dim=1024):
...
class DownsamplingBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, number_of_diffusions, 
                 kernel_size=5, n_heads=None, hidden_dim=None, text_embed_dim=1024):
...
class UpsamplingBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, number_of_diffusions,
                 kernel_size=5, up_dim=None, n_heads=None, hidden_dim=None, text_embed_dim=1024):
    ...
    def forward(self, x, h, t, text_embed):
        x = self.up(x)
        if h is not None:
            x, h = center_crop_to_match(x, h)
            x = torch.cat([x, h], dim=1)
        out = self.block(x, t, text_embed)
        return out
def center_crop_to_match(x, h):
    """
    将 x 和 h 的最后一维裁剪成相同长度（取最小值），中心对齐。
    输入格式为 (B, C, L)
    """
    len_x = x.shape[-1]
    len_h = h.shape[-1]
    target_len = min(len_x, len_h)

    def crop(tensor, target_len):
    ...
    return crop(x, target_len), crop(h, target_len)
class BottleneckNet(nn.Module):
    def __init__(self, n_channels, number_of_diffusions,
                 kernel_size=3, n_heads=4, hidden_dim=16, text_embed_dim=1024):
...
class ECGnocondition(nn.Module):
    def __init__(self, number_of_diffusions, kernel_size=3, num_levels=5, n_channels=4, text_embed_dim=1024):
