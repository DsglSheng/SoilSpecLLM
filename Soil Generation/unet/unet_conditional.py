...
class CrossAttention(nn.Module):
    def __init__(self, vector_size, hidden_dim=16, num_heads=4, text_embed_dim=256, value_embed_dim=1):
        ...
        self.value_combiner = nn.Linear(text_embed_dim + 7 * value_embed_dim, vector_size)
    ...
    def forward(self, x, text_embed, dict):
        ...
        if text_embed.shape:
            ...
            # print('combined_embeds的大小：', combined_embeds.shape)
        else:
            print("err_text")

        for key, value in dict.items():
            # print('dict的大小：', value.shape)
...
class Block(nn.Module):
    def __init__(self, n_inputs, n_outputs, number_of_diffusions,
                 kernel_size=5, n_heads=4, hidden_dim=16, text_embed_dim=256):
...
class DownsamplingBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, number_of_diffusions, 
                 kernel_size=5, n_heads=None, hidden_dim=None, text_embed_dim=256):
...
class UpsamplingBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, number_of_diffusions,
                 kernel_size=5, up_dim=None, n_heads=None, hidden_dim=None, text_embed_dim=256):
    ...
    #     x = self.up(x)
    #     if h is not None:
    #         print(f"Upsample: x.shape = {x.shape}, h.shape = {h.shape}")
...
class BottleneckNet(nn.Module):
    def __init__(self, n_channels, number_of_diffusions,
                 kernel_size=3, n_heads=4, hidden_dim=16, text_embed_dim=256):
...
class ECGconditional(nn.Module):
    def __init__(self, number_of_diffusions, kernel_size=3, num_levels=5, n_channels=4, text_embed_dim=256):
...
if __name__ == '__main__':
    ...
    text_embed = torch.randn((batch_size, 1, 256))
